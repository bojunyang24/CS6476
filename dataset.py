...
class DataLoader(data.Dataset):
    ...
    def __getitem__(self, idx: int) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        while i < (self.batch_size // 2 + 1):
            ...
            _, height, width = self.imgs_left[img_name].shape
            if row >= height - self.ws or col >= width - self.ws or col - d + d_neg < 0 or col - d + d_neg >= width - self.ws:
              continue
